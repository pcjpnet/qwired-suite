 ...
 ***************************************************************************/
#ifndef MIUTRANSACTION_H
#define MIUTRANSACTION_H
...
namespace Qwired {
	enum Errors {
		ErrorOK=0,
		ErrorLoginFailed=100, ErrorPermissionDenied, ErrorObjectNotExists,
		ErrorObjectExists, ErrorObjectProtected, ErrorChecksumMismatch, ErrorAdminAborted };

	enum Flags { FlagResponse=1, FlagListingComplete };

	enum Transactions {
		T_HandshakeRequest=1000, T_LoginRequest, T_SetUserInfo,
		T_PingRequest=1010, T_ServerBannerRequest, T_MotdRequest,
...
public:
	...
	MiuTransaction();
	MiuTransaction(const quint32 t);
	~MiuTransaction();
	...
	MiuTransaction toResponse() const;

...
void WiredSocket::handleTransaction(const QWTransaction & t) {
	...
	} else if(t.type == 2014) { // Conferences list response
		QWClassPrivateChat chat;
		chat.pChatId = t.getObjectInt("cid");
		chat.pTopic = t.getObjectString("topic");
		chat.pUserCount = t.getObjectInt("users");
		chat.pPermanent = t.getObjectInt("type")==1;
		chat.pPassword = t.getObjectString("protected"); // "1" if a password is set, "0" if not
...
// 			emit conferenceListReceived(pConferences);
		...
		QStringList chatLines = t.getObjectString("text").split("\n");
		QStringListIterator i(chatLines);
		while(i.hasNext()) { QString line = i.next();
			emit onServerChat(t.getObjectInt("cid"), t.getObjectInt("uid"), line, t.getObjectInt("emote"));
		}
